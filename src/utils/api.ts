...
// =============================================================================
// PUBLIC FRONTEND API CALLS (DENGAN FALLBACK AMAN)
// =============================================================================
...
export async function runDatabaseSetup(): Promise<{ success: boolean; message: string; tables?: string[] }> {
  try {
    const res = await fetch(`${API_BASE}/setup`, { method: 'POST', headers: getAuthHeaders() });
    const text = await res.text();
    let json: any = null;
    try {
      json = JSON.parse(text);
...
export async function getBookings(status?: string): Promise<BookingRecord[]> {
  try {
    ...
    const res = await fetch(`${API_BASE}/bookings?${params.toString()}`, { headers: getAuthHeaders() });
