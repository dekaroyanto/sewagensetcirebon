...
export const Footer: React.FC<FooterProps> = ({ onNavigate }) => {
  return (
    <footer
      id="kontak"
      className="bg-slate-100 dark:bg-slate-900 text-slate-700 dark:text-slate-300 pt-16 pb-12 border-t border-slate-300 dark:border-slate-800 transition-colors"
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="pt-8 border-t border-slate-200 dark:border-slate-800/80 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-slate-600 dark:text-slate-400 text-center sm:text-left">
          <div>
            © {new Date().getFullYear()} <strong>{COMPANY_INFO.name}</strong>.
            Hak Cipta Dilindungi. Kota Cirebon, Jawa Barat, Indonesia.
          </div>
